package $organization$.$name__word$.infrastructure.logs

import scala.util.parsing.json.{JSONFormat, JSONObject}

import $organization$.$name__word$.infrastructure.logs.SysLogLevel.SysLogLevel
...
class GelfLogger(requestId: Option[UUID] = None) {
  ...
  private def apply(message: String, fullMessage: Option[String] = None, logLevel: SysLogLevel, mapArgs: Map[String, Any] = Map()) = {
    ...
    JSONObject(default ++ customMap).toString(JSONFormat.defaultFormatter)
  }
}
